...
namespace anbox {
namespace container {
LxcContainer::LxcContainer(const Security &security, const network::Credentials &creds)
    : state_(State::inactive), container_(nullptr),  security_(security), creds_(creds) {
  utils::ensure_paths({
...
void LxcContainer::start(const Configuration &configuration) {
  ...
  if (security_ == Security::Unprivileged)
    setup_id_maps();

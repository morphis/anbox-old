...
std::shared_ptr<LoopDevice> LoopDeviceAllocator::new_device() {
  ...
  if (ctl_fd < 0)
    throw std::system_error{errno, std::system_category(), "Could not open /dev/loop-control"};

  DeferAction close_ctl_fd{[&]() { ::close(ctl_fd); }};

  const auto device_nr = ::ioctl(ctl_fd, LOOP_CTL_GET_FREE);
  if (device_nr < 0)
    throw std::system_error{errno, std::system_category(), "Could not query next free loop device"};

...
GLRendererServer::GLRendererServer(const Config &config, const std::shared_ptr<wm::Manager> &wm)
  ...
  if (config.driver == Config::Driver::Translator) {
    ...
    gl_libs.egl_path = (translator_dir / "libEGL_translator.so").string();
    gl_libs.glesv1_path = (translator_dir / "libGLES_CM_translator.so").string();
    gl_libs.glesv2_path = (translator_dir / "libGLES_V2_translator.so").string();
  }
